...
--CREATE PROCEDURE AddUser(ID IN NUMBER) 
CREATE OR REPLACE PROCEDURE AddUser(
    UserName Reviewer.name%type,
    RestaurantName  Restaurant.name%type,
    Rating Rating.stars%type,
    RatingDate Rating.ratingDate%type
)AS
    r1_count integer;
    r2_count integer;
    v_id integer;
    r_id integer;
BEGIN
    select count(*) into r1_count from Reviewer where Reviewer.name=UserName;
    select count(*) into r2_count from Rating where Rating.stars=Rating;
    select Reviewer.vID into v_id from Reviewer inner join Rating on Reviewer.vID=Rating.vID where Reviewer.name = Username;
    select Rating.rID into r_id from Rating inner join Restaurant on Rating.rID = Restaurant.rID where Restaurant.name = RestaurantName;
    if r1_count<1 then
        insert into Reviewer values(2009,UserName);
    elsif r2_count<1 then
        insert into Rating values(r_id,v_id,Rating,RatingDate);
    else 
        DBMS_OUTPUT.PUT_LINE('User ' || v_id || ' is already authorized!');
    end if;   
END;
/
...
CREATE or REPLACE TRIGGER trig_InsertRating_Ratings
after INSERT or update ON Rating
for each row
Begin
    DBMS_OUTPUT.PUT_LINE('execute --'); 
    insert into Top5Restaurant values()
End;
/


call AddUser("Jade Court","Sarah M.", 4, "08/17/2017");
call AddUser("Shanghai Terrace","Cameron J.", 5, "08/17/2017");
call AddUser("Rangoli","Vivek T.",3,"09/17/2017");
call AddUser("Shanghai Inn","Audrey M.",2,"07/08/2017");
call AddUser("Cumin","Cameron J.", 2, "09/17/2017");



    
--select * from Restaurant A inner join rating B on A.rID=B.rID;